...
export async function addRole(role) {
    return await db.roles.add(role);
}

export async function updateRole(id, updates) {
    return await db.roles.update(id, updates);
}
...
// 阅读记录操作（避免使用复杂索引，数据量小，直接过滤）
export async function addReadingRecord(record) {
    return await db.reading_records.add(record);
}

export async function updateReadingRecord(id, updates) {
    return await db.reading_records.update(id, updates);
}

export async function getReadingRecordsByBook(bookId) {
    return await db.reading_records.where('bookId').equals(bookId).toArray();
}
