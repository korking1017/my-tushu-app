import { db, initDefaultRoles, addBook, updateBook, deleteBook, getAllBooks, getBookById, getAllRoles, addReadingRecord, updateReadingRecord, getUnfinishedReadingRecord, getReadingRecordsByBook, getCompletedReadingCountByRole, getAllCompletedRecords, getAllCategories, addCategory, updateCategory, deleteCategory } from './db.js';
...
// 添加/编辑图书表单（包含类别下拉和新类别快速添加）
async function showAddBookForm(book = null) {
    const isEdit = !!book;
    const allCategories = await getAllCategories();
    const categoryOptions = allCategories.map(c => `<option value="${escapeHtml(c.name)}" ${book?.category === c.name ? 'selected' : ''}>${escapeHtml(c.name)}</option>`).join('');
        ...
        <form id="book-form" class="space-y-3">
            ...
            <div>
                ...
                <div class="flex space-x-2">
                    <select id="category-select" class="flex-1 border rounded p-2">
                        <option value="">无</option>
                        ${categoryOptions}
                    </select>
                    <button type="button" id="add-category-btn" class="bg-green-500 text-white px-3 rounded">+ 新类别</button>
                </div>
                <input type="hidden" id="category" value="${escapeHtml(book?.category || '')}">
            </div>
            <div>
    ...
    if (fetchInfoBtn) {
        fetchInfoBtn.onclick = async () => {
            ...
            const originalText = fetchInfoBtn.innerText;
            fetchInfoBtn.innerText = '获取中...';
            ...
            try {
                ...
                fetchInfoBtn.innerText = originalText;
    ...
    if (addCategoryBtn) {
        addCategoryBtn.onclick = async () => {
            ...
            if (newCat && newCat.trim()) {
                ...
                categorySelect.innerHTML = '<option value="">无</option>' + updated.map(c => `<option value="${escapeHtml(c.name)}">${escapeHtml(c.name)}</option>`).join('');
...
async function showCategoriesView() {
    ...
    let html = `
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold">图书类别管理</h2>
            <button id="add-category-manager-btn" class="bg-blue-500 text-white px-3 py-1 rounded"><i class="fas fa-plus"></i> 新增类别</button>
        </div>
    ...
    for (const cat of categories) {
        html += `
            <div class="bg-white p-3 rounded shadow flex justify-between items-center">
                ...
                <div>
                    <button class="edit-category text-blue-500 mr-2" data-id="${cat.id}" data-name="${escapeHtml(cat.name)}"><i class="fas fa-edit"></i></button>
                    <button class="delete-category text-red-500" data-id="${cat.id}"><i class="fas fa-trash"></i></button>
                </div>
            </div>
        `;
    }
    html += `</div><button id="back-to-settings-from-cat" class="mt-4 bg-gray-500 text-white px-4 py-2 rounded w-full">返回设置</button>`;
    appContainer.innerHTML = html;
    
    document.getElementById('add-category-manager-btn')?.addEventListener('click', () => showAddCategoryDialog());
    document.getElementById('back-to-settings-from-cat')?.addEventListener('click', () => showSettingsView());
    
    document.querySelectorAll('.edit-category').forEach(btn => {
        btn.addEventListener('click', () => {
            const id = parseInt(btn.dataset.id);
            const oldName = btn.dataset.name;
            showEditCategoryDialog(id, oldName);
        });
    });
    document.querySelectorAll('.delete-category').forEach(btn => {
        btn.addEventListener('click', async () => {
            const id = parseInt(btn.dataset.id);
            if (confirm('删除类别不会影响已有图书的分类，但该类别将不再出现在下拉选项中。确定删除吗？')) {
...
async function showAddCategoryDialog() {
    modalContent.innerHTML = `
        <form id="new-category-form">
            <label class="block text-sm font-medium">类别名称</label>
            <input type="text" id="new-cat-name" class="w-full border rounded p-2 mb-4" required>
            <div class="flex justify-end">
                <button type="button" id="cancel-cat" class="mr-2 px-4 py-2 bg-gray-300 rounded">取消</button>
                <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded">保存</button>
            </div>
        </form>
    `;
    ...
    document.getElementById('cancel-cat').onclick = () => modalContainer.classList.add('hidden');
}

async function showEditCategoryDialog(id, oldName) {
    modalContent.innerHTML = `
        <form id="edit-category-form">
            <label class="block text-sm font-medium">类别名称</label>
            <input type="text" id="edit-cat-name" value="${escapeHtml(oldName)}" class="w-full border rounded p-2 mb-4" required>
            <div class="flex justify-end">
                <button type="button" id="cancel-edit-cat" class="mr-2 px-4 py-2 bg-gray-300 rounded">取消</button>
                <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded">保存</button>
            </div>
        </form>
    `;
    ...
    document.getElementById('cancel-edit-cat').onclick = () => modalContainer.classList.add('hidden');
}
...
async function showSettingsView() {
    appContainer.innerHTML = `
        ...
        <div class="space-y-3">
            ...
            <div class="bg-white p-3 rounded shadow">
                <div class="flex justify-between items-center">
                    <div>
                        <h3 class="font-bold">图书类别管理</h3>
                        <p class="text-sm text-gray-500">增删改查图书分类，用于图书录入和筛选</p>
                    </div>
                    <button id="go-categories-btn" class="bg-blue-500 text-white px-3 py-1 rounded">管理</button>
                </div>
            </div>
            <div class="bg-white p-3 rounded shadow">
    ...
    document.getElementById('go-categories-btn')?.addEventListener('click', () => showCategoriesView());
